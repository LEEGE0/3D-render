...
- [x] **Step 1: stepped evaluation RED 테스트를 작성한다**
...
- [x] **Step 3: left-hold 타입과 평가를 최소 구현한다**
...
- [x] **Step 4: Action CRUD와 stale/no-op RED 테스트를 작성한다**
...
- [x] **Step 5: Lock-on CRUD와 target/offset RED 테스트를 작성한다**
...
- [x] **Step 6: immutable CRUD API를 구현한다**
...
- [x] **Step 7: snapshot defensive-copy와 경계 회귀를 보강한다**
...
- [x] **Step 1: v1 migration과 v2 round-trip RED를 작성한다**
...
- [x] **Step 3: schema별 validation과 DTO mapping을 구현한다**
...
- [x] **Step 6: Infrastructure 전체를 실행한다**
...
- [x] **Step 1: command Execute/Undo/Redo RED를 작성한다**
...
- [x] **Step 3: 여섯 command를 transform command와 대칭으로 구현한다**
...
- [x] **Step 4: selection·가용성 RED를 작성한다**
...
- [x] **Step 5: active track와 availability를 구현한다**
...
- [x] **Step 6: action/lock CRUD와 deterministic ID를 구현한다**
...
- [x] **Step 7: shared history와 reconciliation을 일반화한다**
...
- [x] **Step 1: pure lane RED 테스트를 작성한다**
...
- [x] **Step 3: Godot 독립 layout을 구현한다**
...
- [x] **Step 5: lane surface와 controller를 구현한다**
...
- [x] **Step 6: semantic Inspector를 구현한다**
...
- [x] **Step 7: scene node와 수명주기 surface API를 완성한다**
...
- [x] **Step 8: Editor와 skeleton을 실행한다**
...
- [x] **Step 1: shared snapshot projection RED를 작성한다**
...
- [x] **Step 2: overlay layout RED를 작성한다**
...
- [x] **Step 3: pure overlay read model을 구현한다**
...
- [x] **Step 5: WorldView OverlayRoot를 확장한다**
...
- [x] **Step 6: projection·Editor 회귀를 실행한다**
...
- [x] **Step 1: Main exact node lookup과 역순 cleanup을 연결한다**
...
- [x] **Step 2: 실제 UI runtime RED를 추가한다**
...
Fix Round 1에서는 `InspectorPanel/HistoryToolbar`의 global Undo/Redo를 Action/Lock-on marker 활성 상태에서 직접 누른다. paused에서 가능한 상태를 먼저 증명한 뒤 Action/Lock-on Add·Apply·Delete와 Undo/Redo 각각의 재생 중 signal이 잠금 사유를 내고 불변인지 검증한다.

Fix Round 2에서는 실제 surface mouse signal로 서로 다른 시각의 Action → Lock-on → Action marker를 왕복하고 target Inspector만 표시되는지 확인한다. Application observer 테스트는 target active track과 full-frame payload가 같은 event에서 보이는지 양방향으로 고정하며, 동일 시각 재게시와 재진입 `Conflict`의 이전 active track 복원도 함께 검증한다.

Fix Round 3에서는 seek notification 뒤 target frame 제거와 actor selection 해제를 재현한다. 호출 전 time/playing, actor, active track과 세 selected ID/full frame을 캡처하고 최신 document 기준으로 selection/time/availability를 원자적으로 복구하며 rollback notification marker 재진입은 bounded `Conflict`로 막는다. same-time Application payload는 양방향 정확히 1회인지 확인하고, Godot는 실제 slider/Add/surface signal로 0.2초 Action↔Lock-on Inspector를 왕복한다.

Fix Round 4에서는 호출 시 active frame이 observer에 의해 다른 시각으로 이동해도 frame 시각이 아니라 호출 time/playing을 복구하고, 세 캡처 ID를 각각 최신 document full frame으로 유지한다. rollback 게시 중 actor null→원래 actor 재선택이 만든 Transform 문맥 알림 뒤에는 원래 active Action/Lock-on payload를 실제 actor/ID/full frame과 함께 다시 게시한다. `Playback.Changed`와 rollback 알림은 재진입 state를 재귀 호출하지 않는 FIFO로 처리하고 32회 안에 안정화되지 않으면 명시적 예외를 낸다. actor null/다른 actor 보존과 same-time 양방향 정확히 1회 계약은 그대로 유지한다.
